...
def create_gradio_app():
    ...
    with gr.Blocks(title="Knowledge Graph MCP Server", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Tab("🔎 Search"):
            ...
            gr.Markdown("---")
            gr.Markdown("### Search by type and name (with fuzzy matching)")
            gr.Markdown("*Supports partial matching: searching 'Embed' will find 'BertEmbeddings', 'RotaryEmbedding', etc.*")
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    search_fuzzy = gr.Checkbox(label="Enable Fuzzy Matching", value=True)
            ...
            search_type_btn.click(
                ...
                inputs=[search_type, search_name, search_type_limit, search_fuzzy],
        ...
        with gr.Tab("🏗️ Structure"):
            ...
            gr.Markdown("---")
            gr.Markdown("### List files in directory (with glob patterns)")

            with gr.Row():
                with gr.Column():
                    files_dir_path = gr.Textbox(label="Directory Path", placeholder="e.g., src/models (empty for all)")
                    files_pattern = gr.Textbox(label="Glob Pattern", value="*", placeholder="e.g., *.py, test_*.py")
                    files_recursive = gr.Checkbox(label="Search Recursively", value=True)
                    files_limit = gr.Slider(minimum=1, maximum=200, value=50, step=1, label="Result Limit")
                    files_btn = gr.Button("List Files", variant="primary")
                with gr.Column():
                    files_output = gr.Textbox(label="Files", lines=20, max_lines=30)

            files_btn.click(
                fn=list_files_in_directory,
                inputs=[files_dir_path, files_pattern, files_recursive, files_limit],
                outputs=files_output
            )

        with gr.Tab("🔬 Discovery"):
            gr.Markdown("### Concept Overview")
            gr.Markdown("Get a high-level overview of how a concept is implemented across the codebase.")

            with gr.Row():
                with gr.Column():
                    concept_query = gr.Textbox(label="Concept", placeholder="e.g., embedding, authentication, cache")
                    concept_limit = gr.Slider(minimum=5, maximum=50, value=15, step=1, label="Results per Category")
                    concept_btn = gr.Button("Get Concept Overview", variant="primary")
                with gr.Column():
                    concept_output = gr.Textbox(label="Concept Overview", lines=25, max_lines=35)

            concept_btn.click(fn=get_concept_overview, inputs=[concept_query, concept_limit], outputs=concept_output)

            gr.Markdown("---")
            gr.Markdown("### Find Classes Inheriting From")
            gr.Markdown("Find all classes that inherit from a specific base class.")

            with gr.Row():
                with gr.Column():
                    inherit_base = gr.Textbox(label="Base Class Name", placeholder="e.g., nn.Module, PreTrainedModel")
                    inherit_limit = gr.Slider(minimum=1, maximum=100, value=20, step=1, label="Result Limit")
            ...
            inherit_btn.click(fn=find_classes_inheriting_from, inputs=[inherit_base, inherit_limit], outputs=inherit_output)

            gr.Markdown("---")
            gr.Markdown("### Find Files Importing")
            gr.Markdown("Find all files that import a specific module or entity.")

            with gr.Row():
                with gr.Column():
                    import_module = gr.Textbox(label="Module/Entity Name", placeholder="e.g., torch, numpy, MyClass")
                    import_limit = gr.Slider(minimum=1, maximum=100, value=30, step=1, label="Result Limit")
                    import_btn = gr.Button("Find Importing Files", variant="primary")
                with gr.Column():
                    import_output = gr.Textbox(label="Files Importing", lines=20, max_lines=30)

            import_btn.click(fn=find_files_importing, inputs=[import_module, import_limit], outputs=import_output)
        ...
        with gr.Tab("🎯 Entities"):
            ...
            with gr.Row():
                with gr.Column():
                    entities_limit = gr.Slider(minimum=1, maximum=200, value=50, step=1, label="Results per Page")
                    entities_page = gr.Number(value=1, minimum=1, step=1, label="Page Number", precision=0)
            ...
            def list_entities_wrapper(limit, page, entity_type, declared_filter):
                ...
                return list_all_entities(limit, int(page), entity_type_val, declared_val)

            entities_btn.click(
                fn=list_entities_wrapper,
                inputs=[entities_limit, entities_page, entities_type_filter, entities_declared_filter],
